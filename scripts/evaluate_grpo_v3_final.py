#!/usr/bin/env python3
"""GRPO-V3 final evaluation: five models on four datasets, greedy decoding, exact accuracy.
...
Models:
  ...
  D. GRPO-V2 Best      outputs/grpo_v2_kl001/checkpoint-600
  E. GRPO-V3 Best      <this round's best checkpoint>

Datasets:
  v2_val            data/processed/v2_answer_only_val.jsonl
  v2_test           data/processed/v2_answer_only_test.jsonl
  grpo_v2_holdout   data/processed/grpo_v2_final_holdout.jsonl   (already opened in V2)
  grpo_v3_holdout   data/processed/grpo_v3_final_holdout.jsonl   (fresh, primary)

The task metric is always EXACT accuracy. The partial reward is a training signal only
and is never used as a final metric.

Usage:
  python scripts/evaluate_grpo_v3_final.py --grpo-v3-checkpoint outputs/grpo_v3_partial/checkpoint-XXX
"""
...
def evaluate(model, tokenizer, rows: list[dict], output: Path, device: str, batch_size: int, max_new_tokens: int) -> dict:
    ...
    for label in LABELS:
        ...
        per_class[label] = {"support": total, "accuracy": (matrix[label][label] / total) if total else None}
...
def main() -> None:
    ...
    parser.add_argument("--grpo-v2-checkpoint", type=Path, default=Path("outputs/grpo_v2_kl001/checkpoint-600"))
    parser.add_argument("--grpo-v3-checkpoint", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/grpo_v3_final"))
    parser.add_argument("--output", type=Path, default=Path("outputs/grpo_v3_final_metrics.json"))
    ...
    datasets = {
        ...
        "grpo_v3_holdout": Path("data/processed/grpo_v3_final_holdout.jsonl"),
    }
    models = {
        ...
        "grpo_v3_best": args.grpo_v3_checkpoint,
    }
