#!/usr/bin/env python3
"""Generate the GRPO-V3 fresh final holdout (2000) with a seed unseen by any earlier split.

The GRPO-V2 fresh holdout has already been opened for the H2 analysis, so GRPO-V3 needs
its own. Same statement-first generator (generate_puzzle_v2) as every earlier split, and
de-duplicated against every puzzle that exists so far:
...
def generate_split(split: str, count: int, seed: int, used_keys: set[str]) -> list[dict]:
    ...
    while len(rows) < count:
        ...
        if key in used_keys:
            continue
        used_keys.add(key)
        puzzle["id"] = f"kk_grpo_v3_{split}_{len(rows):06d}"
        rows.append(puzzle)
        if len(rows) % 500 == 0:
...
def main() -> None:
    ...
    raw_dir = args.output_dir / "raw_grpo_v3"
    ...
    processed_path = processed_dir / "grpo_v3_final_holdout.jsonl"
    ...
    overlap = {
        "vs_legacy_raw": {str(path): count_overlap(path) for path in sources["legacy_raw"]},
        "vs_v2_raw": {str(path): count_overlap(path) for path in sources["v2_raw"]},
        "vs_grpo_v1_raw": {str(path): count_overlap(path) for path in sources["grpo_v1_raw"]},
        "vs_grpo_v2_raw": {str(path): count_overlap(path) for path in sources["grpo_v2_raw"]},
    }
    ...
    manifest = {
        ...
        "generation_rule": "statement-first, exact solve, retain solution_count == 1; dedup against legacy+v2+grpo-v1+grpo-v2+self",
        "dedup_universe": {
    ...
    (processed_dir / "grpo_v3_manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
