#!/usr/bin/env python3
"""Evaluate V1 and V2 best adapters on legacy and v2 test sets."""
...
def evaluate(model, tokenizer, rows: list[dict], output: Path, device: str, batch_size: int, max_new_tokens: int) -> dict:
    ...
    return {
        "count": len(records),
        "exact": sum(record["correct"] for record in records),
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/v2_eval"))
    ...
    models = {
        "V1": Path("outputs/sft_answer_only_10ep_p800"),
        "V2-1K": Path("outputs/sft_v2_1k_p800/checkpoint-315"),
        "V2-5K": Path("outputs/sft_v2_5k_p800/checkpoint-1565"),
    }
    datasets = {
        "legacy_test": Path("data/processed/answer_only_test.jsonl"),
    ...
    for model_name, adapter in models.items():
        ...
        for dataset_name, path in datasets.items():
            ...
            output = args.output_dir / f"{model_name.lower().replace('-', '_')}_{dataset_name}.jsonl"
